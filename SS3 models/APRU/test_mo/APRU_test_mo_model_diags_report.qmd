...
for(i in 1:n.indices){
  ...
  SSplotRunstest(report., subplots = "cpue", add = TRUE, indexselect = i, ylim = c(-.5,.5), verbose = F)
}
...
### Length Comp
```{r}
rmse <- SSrmse(report., quants = "len")$RMSE %>% gt()
rmse.cap <- rmse$RMSE.perc[1]
```
...
#| fig-align: center
#| layout-ncol: 2
#| fig-cap: RMSE for length composition was {rmse.cap}

#sspar(mfrow = c(1,2))
SSplotComps(report., subplots = 8, fleets = 1, new = F, verbose = F)
runs <- SSplotRunstest(report., subplots = "len", add = TRUE, indexselect = 1, ylim = c(-.3,.3), verbose = F)
#SSplotJABBAres(report., subplots = "len", add = TRUE, indexselect = 1, ylim = c(-.4,.4))
```
...
```{r}
#| eval: false
ymax <- max(max(report.$Kobe$F.Fmsy), 1)
ymin <- min()
xmax <- max(report.$Kobe$B.Bmsy, 1)


## Kobe plot
report.$Kobe %>% 
  ggplot() + 
  geom_rect(xmin = 0.0, xmax = 1.0, ymin = 0.0, ymax = ymax) +
  geom_rect(xmin = )
