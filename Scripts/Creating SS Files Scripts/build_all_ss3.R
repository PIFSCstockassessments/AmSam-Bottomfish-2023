...
Species.List <- list.files(file.path(root_dir, "SS3_models"))[-9]
species <- Species.List[1]


### Run Stock Synthesis ####
file.copy(file.path(root_dir, "SS3_models", "TEMPLATE_FILES", "ss_opt_win.exe"), 
          file.path(root_dir, "SS3_models", species))
run_SS_models(dirvec = file.path(root_dir, "SS3_models", species), 
...
rmarkdown::render("~/AmSam-Bottomfish-2023/Scripts/Creating SS Files Scripts/model_diags_report.Rmd", 
                  ...
                  output_dir =  file.path(root_dir, "SS3_models", species, scenario),
                  params = list(
...
report. <- SS_output(file.path(root_dir, "SS3_models", species, scenario))
SSplotJABBAres(report., subplots = "cpue", add = TRUE)
report$warnings
report$parameters %>% filter(str_detect(Status, "HI|LO"))
SS_plots(report, dir = file.path(root_dir, "SS3_models", species))
## Do Retrospectives
SS_doRetro(masterdir=file.path(root_dir, "SS3_models", "TEMPLATE_FILES"), 
           oldsubdir="", newsubdir="Retrospectives", years=0:-5)


## Do Profiling
## Create directory and copy inputs
dir.profile <- file.path(root_dir, "SS3_models", "TEMPLATE_FILES", "R0_Likelihood")
copy_SS_inputs(dir.old = file.path(root_dir, "SS3_models", "TEMPLATE_FILES"),
               dir.new = dir.profile,
               create.dir = TRUE,
               overwrite = TRUE,
               recursive = TRUE,
               ...
               copy_exe = TRUE,
               copy_par = FALSE,
               dir.exe = file.path(root_dir, "SS3_models", "TEMPLATE_FILES"),
               verbose = TRUE)
...
rmarkdown::render("~/AmSam-Bottomfish-2023/Scripts/Creating SS Files Scripts/model_diags_report.Rmd", 
                  ...
                  output_dir =  file.path(root_dir, "SS3_models", species),
                  params = list(
                    species = paste0(species),
                    scenario = "Base",
                    report = "../../SS3_models"
