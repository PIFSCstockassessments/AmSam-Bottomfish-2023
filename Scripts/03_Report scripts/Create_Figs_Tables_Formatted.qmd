...
```{r}
#| label: fig-indexfits
#| fig-cap: !expr 'sprintf("RMSE is %.2f", rmse)'
#| out-width: 11cm
#| out-height: 7.88cm
#| fig-align: center
...
```{r}
#| label: fig-jitter
#| fig-cap: "Jitter analysis was run to ensure model converged at global solution."
#| layout-ncol: 2
#| out-width: 15cm
...
Results.ft <- compose(Results.ft, j ="Model", i = 4, part = "body",
                      value = as_paragraph(as_i("L"), as_sub("inf"), "= XX cm"))
...
Results.ft <- compose(Results.ft, j ="Model", i = 10, part = "body",
                      value = as_paragraph("No historical catch"))
if(nrow(Results) > 10){
  Results.ft <- compose(Results.ft, j ="Model", i = 11, part = "body",
                      value = as_paragraph("No hermaphroditism"))
}
Results.ft <- autofit(Results.ft)
Results.ft
...
#| label: fig-altmods-1
#| out-width: 16.5cm
#| fig-cap: "Sensitivity analyses showing differences in spawning biomass, fishing mortality, recruitment, and final year stock status (Kobe plot) under different scenarios."

path <- c(file.path("../../..", "SS3 final models", params$species, "00_Alternate_Mods_Figs_Tables", "Sensitivity1.png"),
          file.path("../../..", "SS3 final models", params$species, "00_Alternate_Mods_Figs_Tables", "Sensitivity2.png"))
include_graphics(path)
